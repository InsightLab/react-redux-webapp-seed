...
type ReduxDispatchActionCreator<Payload, Result> = (
  payload?: Payload
) => (dispatch: ReduxDispatch<ReduxAction<Payload>>) => Promise<Result>;
...
type ActionCreators<Payload, Result> = {
  [index: string]: ReduxActionCreator<Payload, Result>;
};
...
type SagaGenerator<T> = Generator<
  CallEffect<T> | PutEffect<ReduxAction<T>>,
  void,
  T
>;
